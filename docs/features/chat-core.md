...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-flow-chat-core-send-message`
...
**Steps**:
1. [ ] - `p1` - User submits message via POST /api/v1/conversations/:id/messages - `inst-submit-message`
2. [ ] - `p1` - Validate user is authenticated and owns conversation - `inst-validate-auth-owner`
3. [ ] - `p1` - Algorithm: check rate limit using `cpt-ai-chat-via-cyber-pilot-algo-chat-core-rate-limit` - `inst-check-rate-limit`
4. [ ] - `p1` - **IF** rate limit exceeded **THEN** return 429 Too Many Requests - `inst-rate-limit-exceeded`
5. [ ] - `p1` - DB: INSERT INTO messages (conversation_id, role='user', content) - `inst-save-user-message`
6. [ ] - `p1` - DB: UPDATE conversations SET message_count = message_count + 1, updated_at = NOW() - `inst-update-conversation`
7. [ ] - `p1` - Algorithm: build context using `cpt-ai-chat-via-cyber-pilot-algo-chat-core-build-context` - `inst-build-context`
8. [ ] - `p1` - Set SSE headers (Content-Type: text/event-stream) - `inst-set-sse-headers`
9. [ ] - `p1` - Algorithm: stream LLM response using `cpt-ai-chat-via-cyber-pilot-algo-chat-core-stream-llm` - `inst-stream-llm`
10. [ ] - `p1` - **FOR EACH** token received from LLM - `inst-token-loop`
    1. [ ] - `p1` - Send SSE event with token data - `inst-send-sse-token`
    2. [ ] - `p1` - Accumulate token to response buffer - `inst-accumulate-token`
11. [ ] - `p1` - DB: INSERT INTO messages (conversation_id, role='assistant', content, metadata) - `inst-save-assistant-message`
12. [ ] - `p1` - DB: UPDATE conversations SET message_count = message_count + 1, updated_at = NOW() - `inst-update-conversation-final`
13. [ ] - `p1` - Algorithm: auto-generate title if first response using `cpt-ai-chat-via-cyber-pilot-algo-conversation-mgmt-auto-title` - `inst-auto-title`
14. [ ] - `p1` - Send SSE done event with message ID - `inst-send-sse-done`

### Stop Generation Flow

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-flow-chat-core-stop`
...
**Steps**:
1. [ ] - `p1` - User clicks stop or POST /api/v1/conversations/:id/stop - `inst-submit-stop`
2. [ ] - `p1` - Validate user is authenticated and owns conversation - `inst-validate-auth-stop`
3. [ ] - `p1` - Set abort flag in active generation context (Redis) - `inst-set-abort-flag`
4. [ ] - `p1` - **RETURN** 200 OK with acknowledgment - `inst-return-stop-ack`

### Regenerate Response Flow

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-flow-chat-core-regenerate`
...
**Steps**:
1. [ ] - `p1` - User clicks regenerate or POST /api/v1/conversations/:id/regenerate - `inst-submit-regenerate`
2. [ ] - `p1` - Validate user is authenticated and owns conversation - `inst-validate-auth-regen`
3. [ ] - `p1` - Algorithm: check rate limit using `cpt-ai-chat-via-cyber-pilot-algo-chat-core-rate-limit` - `inst-check-rate-limit-regen`
4. [ ] - `p1` - DB: SELECT last message WHERE role='assistant' ORDER BY created_at DESC - `inst-find-last-assistant`
5. [ ] - `p1` - **IF** no assistant message found **THEN** return 400 Bad Request - `inst-no-message-to-regen`
6. [ ] - `p1` - DB: DELETE FROM messages WHERE id = :last_assistant_id - `inst-delete-last-assistant`
7. [ ] - `p1` - DB: UPDATE conversations SET message_count = message_count - 1 - `inst-decrement-count`
8. [ ] - `p1` - Algorithm: build context using `cpt-ai-chat-via-cyber-pilot-algo-chat-core-build-context` - `inst-build-context-regen`
9. [ ] - `p1` - Set SSE headers - `inst-set-sse-headers-regen`
10. [ ] - `p1` - Algorithm: stream LLM response using `cpt-ai-chat-via-cyber-pilot-algo-chat-core-stream-llm` - `inst-stream-llm-regen`
11. [ ] - `p1` - DB: INSERT INTO messages (conversation_id, role='assistant', content, metadata) - `inst-save-regen-message`
12. [ ] - `p1` - DB: UPDATE conversations SET message_count = message_count + 1, updated_at = NOW() - `inst-update-conversation-regen`
13. [ ] - `p1` - Send SSE done event - `inst-send-sse-done-regen`

### Rate Limit Check Flow

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-flow-chat-core-rate-limit-check`
...
**Steps**:
1. [ ] - `p1` - Extract user ID and IP from request - `inst-extract-identifiers`
2. [ ] - `p1` - REDIS: GET rate_limit:user:{user_id} - `inst-get-user-limit`
3. [ ] - `p1` - REDIS: GET rate_limit:ip:{ip} - `inst-get-ip-limit`
4. [ ] - `p1` - **IF** user limit exceeded OR ip limit exceeded **THEN** return 429 - `inst-limit-exceeded`
5. [ ] - `p1` - REDIS: INCR rate_limit:user:{user_id} with TTL - `inst-incr-user-limit`
6. [ ] - `p1` - REDIS: INCR rate_limit:ip:{ip} with TTL - `inst-incr-ip-limit`
7. [ ] - `p1` - Set rate limit headers (X-RateLimit-Remaining, X-RateLimit-Reset) - `inst-set-rate-headers`

## 3. Processes / Business Logic (CDSL)

### Build Context

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-algo-chat-core-build-context`
...
**Steps**:
1. [ ] - `p1` - Load system prompt from configuration - `inst-load-system-prompt`
2. [ ] - `p1` - DB: SELECT messages FROM conversation ORDER BY created_at ASC - `inst-load-messages`
3. [ ] - `p1` - Algorithm: count tokens for each message using `cpt-ai-chat-via-cyber-pilot-algo-chat-core-count-tokens` - `inst-count-message-tokens`
4. [ ] - `p1` - Calculate total tokens including system prompt - `inst-calc-total-tokens`
5. [ ] - `p1` - **WHILE** total tokens > max tokens - `inst-truncation-loop`
    1. [ ] - `p1` - Remove oldest non-system message - `inst-remove-oldest`
    2. [ ] - `p1` - Recalculate total tokens - `inst-recalc-tokens`
6. [ ] - `p1` - Build context array: [system, ...messages] - `inst-build-array`
7. [ ] - `p1` - **RETURN** context array - `inst-return-context`

### Stream LLM Response

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-algo-chat-core-stream-llm`
...
**Steps**:
1. [ ] - `p1` - Initialize OpenAI client with API key - `inst-init-openai`
2. [ ] - `p1` - Create chat completion request with stream=true - `inst-create-request`
3. [ ] - `p1` - **TRY** - `inst-try-stream`
    1. [ ] - `p1` - **FOR EACH** chunk from stream - `inst-chunk-loop`
        1. [ ] - `p1` - **IF** abort signal set **THEN** break - `inst-check-abort`
        2. [ ] - `p1` - Extract token from chunk.choices[0].delta.content - `inst-extract-token`
        3. [ ] - `p1` - **YIELD** token - `inst-yield-token`
4. [ ] - `p1` - **CATCH** error - `inst-catch-error`
    1. [ ] - `p1` - **IF** rate limit error **THEN** throw RateLimitError - `inst-handle-rate-limit`
    2. [ ] - `p1` - **IF** timeout error **THEN** throw TimeoutError - `inst-handle-timeout`
    3. [ ] - `p1` - Log error and throw LLMError with graceful message - `inst-handle-generic-error`

### Count Tokens

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-algo-chat-core-count-tokens`
...
**Steps**:
1. [ ] - `p1` - Get tiktoken encoding for model - `inst-get-encoding`
2. [ ] - `p1` - Encode text to tokens - `inst-encode-text`
3. [ ] - `p1` - **RETURN** tokens.length - `inst-return-count`

### Apply Rate Limit

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-algo-chat-core-rate-limit`
...
**Steps**:
1. [ ] - `p1` - Define limits: user=60/min, ip=100/min for authenticated, ip=10/min for anonymous - `inst-define-limits`
2. [ ] - `p1` - REDIS: GET rate_limit:user:{user_id}:count - `inst-get-user-count`
3. [ ] - `p1` - REDIS: GET rate_limit:ip:{ip}:count - `inst-get-ip-count`
4. [ ] - `p1` - Calculate remaining = limit - count - `inst-calc-remaining`
5. [ ] - `p1` - **IF** remaining <= 0 **THEN** return { allowed: false, remaining: 0, resetAt } - `inst-return-blocked`
6. [ ] - `p1` - REDIS: INCR with EXPIRE 60s - `inst-incr-count`
7. [ ] - `p1` - **RETURN** { allowed: true, remaining, resetAt } - `inst-return-allowed`

## 4. States (CDSL)

### Message Generation State

- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-state-message-generation`
...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-send-message-endpoint`
...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-stop-endpoint`
...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-regenerate-endpoint`
...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-context-management`
...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-llm-adapter`
...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-rate-limiting`
...
- [ ] `p1` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-message-persistence`
...
- [ ] `p2` - **ID**: `cpt-ai-chat-via-cyber-pilot-dod-chat-core-error-handling`
